...
year_value <-
  ...
  year_party_value %>% 
    group_by(year) %>% 
    summarise(total_value_year = sum(value))
...
year_party_value_prop <-
  ...
  left_join(
    ...
    # calculate prop
    mutate(prop = value / total_value_year) %>% 
    
    # use select to format output
    select(
      year
      ,name = party_name
      ,total_year_donations = value
      ,prop
    )
