...
# Filter to only those with > 50 seats
# Filter to only include planes which have a dep_time
...
# Sort descending and select top 1
# Store in new table
# Display result

flights_planes_50p_top1 <-
  top_n(
    flights_planes_50p %>%
      group_by(tailnum) %>% 
...
# Right join to top 1
# summarise by destination to get number of flights to each destination
# Bar plot to display results
  
ggplot(
  right_join(flights_planes_50p, flights_planes_50p_top1, by = "tailnum") %>%
    group_by(dest) %>%
    summarise(dest_flight_count = sum(!is.na(dep_time)))
  ,aes(x = dest, y = dest_flight_count, fill = dest)
) +
  ...
  labs(
    ...
    ,title = "Destinations of 50+ seat plane with most flights from NYC in 2013"
  )
```
...
ggplot(
  ...
  labs(
    x = "July"
    ,y = "Temperature"
    ,title = "July 2013 Temperature distribution "
  )

# Scater plot of temperature vs windspeed with temp outliers highlighted

ggplot(
  weather_july_tempoutliers
  ,aes(x = wind_speed, y = temp, colour = temp_outlier)
) +
  geom_point() +
  labs(
    x = "Wind Speed"
    ,y = "Temperature"
    ,title = "July 2013 - Temperature (with outliers) vs Windpseed"
    ,colour = "Temperature Outlier"
  )

# // What is the relationship between `dewp` and `humid`?
  
# Scatter plot of dewp vs humid
...
ggplot(
  ...
  labs(
    ...
    ,y = "Humidity"
    ,title = "Dewpoint Temperature vs Humidity"
  )
...
ggplot(
  ...
  labs(
    x = "Precipitation"
    ,y = "Visibility"
    ,title = "Precipitation vs Visibility"
  )
...
# Filter to return planes whose year value is NA and summarise to count them
...
# inner join flights with dep_time to manufacturers
# Group by manufacturer and get flight count and plane count
# Select top 5 only
...
# Display results of top 5 by plane_count and flight_count
top_n(
  manuf_flightcount_planecount %>% 
...
# join the list of distinct tailnums to planes and sort by year
# Use top n to return lowest year value
# Can use top_n to get only top result
...
# count distinct tailnumbers for flights with a dep_time
...
# group flights with dep_time by origin and month
...
# group flights with dep_time by carrier, origin, and month
# calculate median_arr_delay ignoring na using summarise
...
# Join flight to airlines
# group by carrier name and calculate SFO flight and SFO flights over total flights
# assign to new table
...
# Union the actor columns to so we can get distinct list
# group by movie name and summarise to get count of actors
# Can use top_n to get only top result
...
# union_all eact actor column to get the full list of actors
# group_by and summarize to get counts
...
# group by release_year and summarise to get mean and median age_difference
# use union to put data in normalised form to make plotting easier. Create metric and value column for median and then for mean
...
# group by and summarise to get counts
